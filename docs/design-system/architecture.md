...
Os tokens globais continuam em `app/globals.css`, porque Tailwind 4 e shadcn já estão configurados ali.
...
Hoje os componentes shadcn estão em:

```txt
src/modules/blog-admin/ui/components
```

Esse local limita reuso fora do admin. A migração ideal é:

1. Criar `src/shared/ui/lib/cn.ts`.
2. Mover componentes shadcn para `src/shared/ui/primitives` e `src/shared/ui/feedback`.
3. Atualizar `components.json` para apontar `components`, `ui`, `utils` e `lib` para `src/shared/ui`.
4. Atualizar imports do admin para `@/src/shared/ui`.
5. Manter `admin-theme.css` apenas como tema, não como dono dos componentes.
